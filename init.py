...
def init_data(args):
    ...
    if args.dataset == 'rhm':
        ...
        if args.num_tokens < args.input_size:
            # Only take the last num_tokens positions in the model input.
            # The dataset.rhm_sequences attribute, if present, is intentionally
            # kept full-length so that diagnostics can still know the true RHM
            # sequence and fill non-visible levels with NaNs.
    ...
    if args.mode == 'masked':
        # Hide last feature from input and set it as label.
        dataset.labels = torch.argmax(dataset.features[:, :, -1], dim=1)

        if 'fcn' in args.model:
            # For FCNs remove masked token from the input.
            dataset.features = dataset.features[:, :, :-1]
            args.num_tokens -= 1
        else:
            # For other models replace masked token with a constant unit-norm mask.
    ...
    if 'transformer' in args.model:
        # Transformer requires [batch_size, seq_len, num_channels].
        dataset.features = dataset.features.transpose(1, 2)

    dataset.features = dataset.features.to(args.device)
...
def init_training(model, args):
    ...
    if args.scheduler is None:
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=args.max_epochs)
    elif args.scheduler == 'cosine':
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=args.scheduler_time,
            eta_min=0.1 * args.lr,
        )
    elif args.scheduler == 'warmup':
        scheduler = CosineWarmupLR(optimizer, args.scheduler_time, max_iters=args.max_epochs)
    else:
...
def init_output(model, criterion, train_loader, test_loader, args):
    ...
    Returns:
        dynamics list and best-model dictionary.
    """
...
def init_loglinckpt(step, end, fill=False):
    """
    Initialise checkpoint iterator with i*step until end.

    If fill=True, fill the first step with up to 10 logarithmically spaced points.
    """
    current = step
    checkpoints = []
