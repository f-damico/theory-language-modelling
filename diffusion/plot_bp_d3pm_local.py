#!/usr/bin/env python3
"""Plot helpers for constrained_bp_d3pm_local.py outputs."""
...
def plot_loss_error(
    ...
    *,
    ...
    axs[0].set_ylabel("D3PM x0-prediction CE")
    axs[0].set_title("BP-D3PM denoising test loss")
    ...
    axs[1].set_title("BP-D3PM x0 top-1 error")
...
def plot_generated_rule_errors(
    results_or_path: Dict[str, Any] | str | Path,
    x_key: str = "lambda_values",
    *,
    log_x: bool = True,
    ...
    """Plot generated-sample RHM rule error/validity by level versus lambda."""
    res = _as_results(results_or_path)
    if "generated_error_frac_by_level" not in res:
        raise KeyError(
            "generated_error_frac_by_level not found. Re-run constrained_bp_d3pm_local.py "
            "with compute_generation=True or --compute_generation."
        )
    x, xlabel = _extract_x(res, x_key)
    err = np.asarray(res["generated_error_frac_by_level"], dtype=float)
    valid = np.asarray(res.get("generated_valid_frac_by_level", 1.0 - err), dtype=float)
    mask = _finite_mask(x, err, valid)
    x, err, valid = x[mask], err[mask], valid[mask]
    L = err.shape[1]

    fig, axs = plt.subplots(1, 2, figsize=(13, 4))
    for ell in range(L):
        axs[0].plot(x, err[:, ell], marker="o", label=fr"$\ell={ell+1}$")
    _set_scale(axs[0], x, "x", log_x)
    axs[0].set_xlabel(xlabel)
    axs[0].set_ylabel("fraction of generated samples with rule error")
    axs[0].set_title("D3PM generated-sample RHM errors")
    axs[0].grid(True, which="both", alpha=0.3)
    axs[0].legend()

    for ell in range(L):
        axs[1].plot(x, valid[:, ell], marker="o", label=fr"$\ell={ell+1}$")
    _set_scale(axs[1], x, "x", log_x)
    axs[1].set_xlabel(xlabel)
    axs[1].set_ylabel("fraction compatible")
    axs[1].set_title("D3PM generated-sample RHM compatibility")
    axs[1].grid(True, which="both", alpha=0.3)
    axs[1].legend()
    fig.tight_layout()
    if save_path is not None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot local constrained BP-D3PM results.")
    ...
    if "generated_error_frac_by_level" in res:
        plot_generated_rule_errors(res, x_key=args.x_key, save_path=f"{out_prefix}_generated_rule_errors.png")
